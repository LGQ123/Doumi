...
//        if ([BusinessLogic power].length ==0) {
            cell.imageArr = @[@"视频专区",@"近期活动",@"荣耀榜单",@"签到"];
//        } else {
...
- (void)bannerIndex:(NSInteger)index {
    ...
    NSDictionary *dic = @{@"location":[NSString stringWithFormat:@"%li",index]};
    [MobClick event:@"homeBanner" attributes:dic];
...
- (void)hotCollecIndex:(NSInteger)index {
    ...
    if (index == 0) {
        ...
        NSDictionary *dict = @{@"location" :[NSString stringWithFormat:@"%li",index-1]};
        [MobClick event:@"Icon" attributes:dict];
