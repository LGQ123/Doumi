...
NSString * const ID1 = @"CustomBannerCell";

@interface SDCycleScrollView () <UICollectionViewDataSource, UICollectionViewDelegate,ShareDelegate>
...
- (void)setupMainView
{
    ...
    [mainView registerNib:[UINib nibWithNibName:@"CustomBannerCell" bundle:nil] forCellWithReuseIdentifier:ID1];
...
- (void)setImagePathsGroup:(NSArray *)imagePathsGroup
{
    ...
    NSLog(@"=====%d",_imagePathsGroup.count);
    [self setupPageControl];
...
- (void)setMode:(Hashmaplist *)mode {
    ...
    [_mode.ents enumerateObjectsUsingBlock:^(Ents * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        Ents *ents;
        if ([obj isKindOfClass:[Ents class]]) {
            ents = obj;
        }
        if (ents) {
            [temp addObject:ents];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (!_LGQType) {
        ...
        CustomBannerCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:ID1 forIndexPath:indexPath];
        cell.delegate = self;
        long itemIndex = indexPath.item % self.imagePathsGroup.count;
//        NSString *imagePath = self.imagePathsGroup[itemIndex];
        Ents *mode = self.imagePathsGroup[itemIndex];
        cell.entsMode = mode;
        cell.ID = itemIndex;
        cell.delegate = self;
        return cell;
...
- (void)likeDelegate:(NSInteger)ID {
    if ([self.delegate respondsToSelector:@selector(likeTouch:)]) {
        [self.delegate likeTouch:ID];
    }
...
- (void)shareCell:(NSInteger )ID {
    if ([self.delegate respondsToSelector:@selector(shareTouch:)]) {
        [self.delegate shareTouch:ID];
    }
}
