...
- (RootViewController *)searchResultController
{
    if (!_searchResultController) {
        _searchResultController = [[SearchResultController alloc] init];
        self.searchResultTableView = _searchResultController.searchTableView;
    }
...
- (void)searchBarSearchButtonClicked:(UISearchBar *)searchBar
{
    ...
    switch (self.searchResultShowMode) {
        ...
        case PYSearchResultShowModeEmbed: // 内嵌
            // 添加搜索结果的视图
            self.searchResultController.searchStr = searchBar.text;
            NSLog(@"searchBar.text = %@",searchBar.text);
            [self.view addSubview:self.searchResultController.searchTableView];
            [self.searchResultController request_Api];
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    ...
    return  self.searchHistoryStyle == PYSearchHistoryStyleCell ? self.searchHistories.count>5?5:self.searchHistories.count : 0;
}
